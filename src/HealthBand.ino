...
void sendSensor(){
  ...
  if(temp<36 || temp>38){
    Blynk.virtualWrite(V14, "Abnormal");
  }
  else{
  ...
  Blynk.virtualWrite(V13, beatAvg);
  ...
  if (irValue < 50000){
    ...
    Blynk.virtualWrite(V15, "Not detected");
  }
  
  else{
    if(beatAvg<60 || beatAvg>100){
      Blynk.virtualWrite(V15, "Abnormal");
    }
    else{
    Blynk.virtualWrite(V15, "Normal");
    }
  }
}
